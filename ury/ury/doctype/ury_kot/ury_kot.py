...
class URYKOT(Document):
    ...
    def create_or_update_work_orders(self):
        ...
        company, fg_warehouse = frappe.db.get_value("POS Profile", self.pos_profile, ["company", "warehouse"])
...
def get_all_kots(invoice_id):
	return frappe.get_all("URY KOT", filters={"invoice": invoice_id}, fields=["name", "type", "original_kot"])
...
def handle_full_cancellation(all_kots, existing_wos):
	...
	for wo in existing_wos:
		wo_totals[wo.production_item] = wo_totals.get(wo.production_item, 0) + float(wo.qty or 0)
...
def on_kot_update(doc, method):
	...
	work_orders = frappe.get_all(
		"Work Order", filters={"pos_invoice": doc.invoice}, fields=["name", "status", "docstatus"]
	)

	if not work_orders:
		frappe.logger().info(f"No Work Orders found for invoice {doc.invoice} (KOT {doc.name})")
		return
	...
	for wo in work_orders:
		...
		try:
			...
			if stock_entry_data:
				...
				frappe.logger().info(f"Stock Entry {stock_entry_doc.name} submitted for Work Order {wo.name}")
		except Exception:
			frappe.log_error(frappe.get_traceback(), f"Work Order Stock Entry Error for {wo.name}")
			continue
