...
def sales_invoice_naming(doc, method):
	...
	pos_profile = frappe.db.get_value(
		"POS Profile", doc.pos_profile, ["restaurant_prefix", "restaurant"], as_dict=True
	)
...
def aggregator_unpaid(doc, method):
	if (
		...
		and frappe.db.get_value("Branch", doc.branch, "custom_make_unpaid") == 1
	):
		doc.is_pos = 0
...
class URYSalesInvoice(SalesInvoice):
	...
	def validate_qsr_raw_materials(self, d, qsr_item_groups, missing_materials):
		...
		if self.docstatus.is_draft() and not self.skip_raw_material_validation:
			bom = frappe.db.get_value("BOM", {"item": d.item_code, "is_default": 1, "is_active": 1}, "name")
			if not bom:
				frappe.throw(_("Row #{0}: No default BOM found for QSR Item {1}").format(d.idx, d.item_name))
			...
			for rm_code, rm in leaf_items.items():
				...
				available_qty = get_stock_balance(rm_code, source_warehouse, self.posting_date)
	...
	def get_all_leaf_bom_items(bom, company, qsr_item_groups):
		...
		for rm_code, rm in bom_items.items():
			...
			do_not_explode = (
				frappe.db.get_value("BOM Item", {"parent": bom, "item_code": rm_code}, "do_not_explode") or 0
			)
			...
			if rm_item_group in qsr_item_groups and not do_not_explode:
				...
				if child_bom:
					child_items = URYSalesInvoice.get_all_leaf_bom_items(child_bom, company, qsr_item_groups)
	...
	def validate_invoice(self):
		...
		if self.invoice_printed == 1 and remove_items == 0:
			...
			original_doc = frappe.get_doc("Sales Invoice", self.name)

			# Create dictionaries to store both quantities and names
			original_items = {
				item.item_code: {"qty": item.qty, "name": item.item_name} for item in original_doc.items
			}
			current_items = {item.item_code: {"qty": item.qty, "name": item.item_name} for item in self.items}
			...
			for item_code, item_data in original_items.items():
				if item_code in current_items and current_items[item_code]["qty"] < item_data["qty"]:
			...
			if removed_items or reduced_qty_items:
				error_msg = []
				if removed_items:
					removed_item_names = [original_items[item_code]["name"] for item_code in removed_items]
				...
				frappe.throw(_(f"Cannot modify items after invoice is printed.\n{error_msg}"))
	...
	def validate_invoice_print(self):
		...
		invoice_printed = frappe.db.get_value("Sales Invoice", self.name, "invoice_printed")
		...
		if self.restaurant_table and invoice_printed == 0:
			frappe.throw("Printing the invoice is mandatory before submitting. Please print the invoice.")
	...
	def order_type_update(self):
		if self.restaurant_table:
			if not self.order_type:
				is_take_away = frappe.db.get_value("URY Table", self.restaurant_table, "is_take_away")
				if is_take_away == 1:
		...
		if self.restaurant:
			if self.restaurant_table:
				...
				menu_name = (
					...
					if not frappe.db.get_value("URY Restaurant", self.restaurant, "room_wise_menu")
	...
	def restrict_existing_order(self):
		if self.restaurant_table:
			...
			if invoice_exist:
				frappe.throw(_(f"Table {self.restaurant_table} has an existing invoice"))
	...
	def auto_complete_work_orders(self):
		...
		try:
			...
			for wo in work_orders:
				...
				draft_entry = next((se for se in existing_entries if se.docstatus == 0), None)
				if draft_entry:
					se_doc = frappe.get_doc("Stock Entry", draft_entry.name)
					se_doc.submit()
					succeeded.append(f"{work_order.name} (used existing draft SE {se_doc.name})")

				# Otherwise, skip if already submitted
				elif existing_entries and existing_entries[0].docstatus == 1:
					skipped.append(f"{work_order.name} (SE {existing_entries[0].name} already submitted)")
					...
					if not stock_entry_data:
						failed.append((work_order.name, "make_stock_entry returned None"))
					...
					succeeded.append(f"{work_order.name} (created new SE {stock_entry_doc.name})")
